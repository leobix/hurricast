...
def run_xgb(vision_data, stat_data, window_size, predict_at, max_depth, reduced_ranks = [5,7,10,10]):
    ...
    X_train =concat_stat_viz(x_stat_train, x_viz_train, reduced_ranks)
    X_test =concat_stat_viz(x_stat_test, x_viz_test, reduced_ranks)
    ...
    return np.round(score_xgb,3) , np.round(score_base,3)
    ...
    for max_depth in max_depth_list:
        for steps_in in steps_in_list:
            for steps_out in steps_out_list:
                #run model
                intensity_xgb_score, intensity_base_score = run_xgb(vision_data, stat_data, window_size = steps_in, predict_at = steps_out, max_depth, reduced_ranks = [5,8,10,10])
                #record accuracy
                accuracy = accuracy.append({'past_n_steps': str(steps_in),
                                                  'pred_n_steps': str(steps_out),
                                                  'max_depth': str(max_depth),
                                                  'xgb_intensity_accu': intensity_xgb_score,
                                                  'base_intensity_accu': intensity_base_score}, ignore_index=True) #'dx_xgb_mae':dx_xgb_mae, 'dy_xgb_mae':dy_xgb_mae
